...
node *MM_create_hash_tree_node(size_t size) {
    ...
    if (!newNode) return NULL;  // Return NULL if node allocation fails.
...
void MM_insert_node(node *actualNode, node *newNode) {
    ...
    while (current) {
        ...
        if (comparison == 0) {  // If newNode is smaller, go left.
            current = current->left;
            direction = 0;
        } else if (comparison == 1) {  // If newNode is larger, go right.
...
node *MM_find_minimum(node *current_node) {
    if (!current_node) return NULL;  // Return NULL if node is empty.
...
void MM_transplant(node *from_node, node *to_node) {
    if (!from_node->father) {  // If from_node is the root, replace it with to_node.
        ROOT = to_node;
...
void MM_delete_node(node *node_to_delete) {
    if (!node_to_delete) return;  // Return if node is NULL.
...
node *MM_find_node_by_hash(node *current_node, unsigned char hash[]) {
    if (!current_node) return NULL;  // Return NULL if current_node is NULL.

    int comparison_result = MM_compare_hash(current_node->hash, hash);  // Compare the node's hash with the target hash.
    if (comparison_result == 2) return current_node;  // Return node if hashes match.

    // Recursively search left or right subtree based on comparison result.
    if (comparison_result == 0) return MM_find_node_by_hash(current_node->left, hash);
...
void *API_MM_allocateMem(size_t size) {
    if (size == 0) return NULL;  // Return NULL if size is zero.

    node *new_node = MM_create_hash_tree_node(size);  // Create a new node to manage the memory block.
    if (!new_node) return NULL;  // Return NULL if node creation fails.

    MM_insert_node(ROOT, new_node);  // Insert the node into the tree.
...
void *API_MM_reallocMem(void *ptr, size_t new_size) {
    if (new_size == 0) {
        API_MM_freeMem(ptr);  // Free memory if new size is zero.
        return NULL;
    }

    if (!ptr) return API_MM_allocateMem(new_size);  // Allocate new memory if pointer is NULL.

    unsigned char hash[HASH_BLOCK_SIZE];
    MM_hash_address(ptr, hash);  // Hash the current pointer.

    node *node_to_resize = MM_find_node_by_hash(ROOT, hash);  // Find the node for the current memory.
    if (!node_to_resize) return NULL;  // Return NULL if node not found.

    if (new_size <= node_to_resize->size) {
        node_to_resize->size = new_size;  // Adjust size if new size is smaller.
        return ptr;  // No need to move memory.
    }

    void *new_ptr = API_MM_allocateMem(new_size);  // Allocate new memory for larger size.
    if (!new_ptr) return NULL;  // Return NULL if allocation fails.

    memcpy(new_ptr, ptr, node_to_resize->size);  // Copy the old data to the new block.
    API_MM_freeMem(ptr);  // Free the old memory.

    return new_ptr;  // Return the new memory pointer.
}
