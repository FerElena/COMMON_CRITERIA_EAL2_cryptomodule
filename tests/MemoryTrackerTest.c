...
void test_API_MT_add_tracker_success()
{
    ...
    int index = API_MT_add_tracker(data, sizeof(data), 1);
...
void test_API_MT_add_tracker_failure()
{
    ...
    int index = API_MT_add_tracker(data, 100, 1);
...
void test_API_MT_update_tracker_success()
{
    ...
    int index = API_MT_add_tracker(data, sizeof(data), 0);
    MemoryTracker *tracker = &trackers[index];
    int update_result = API_MT_update_tracker(tracker, updatedData, sizeof(updatedData));
...
void test_API_MT_update_tracker_failure()
{
    ...
    int index = API_MT_add_tracker(data, sizeof(data), 0);
    MemoryTracker *tracker = &trackers[index];
    int update_result = API_MT_update_tracker(tracker, NULL, 150); // Invalid pointer
...
void test_API_MT_zeroize_and_free_all()
{
    ...
    API_MT_add_tracker(data1, sizeof(data1), 1);
    API_MT_add_tracker(data2, sizeof(data2), 1);
