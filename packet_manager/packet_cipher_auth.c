...
int API_PCA_sign_encrypt_packet(unsigned char *data_in, size_t data_in_length, unsigned char *key_AES, unsigned char *key_HMAC, unsigned char *out_data, size_t *out_data_length)
{
	...
	if (aux_buffer_length > data_buffer_sign_encrypt_length )
	{
	...
	memcpy(aux_buffer_pointer,data_in,data_in_len_aux);
	memcpy(aux_buffer_pointer + data_in_len_aux,sign_out,HMAC_SHA256_sign_size);
...
int API_PCA_decrypt_verify_packet(unsigned char *data_in, size_t data_in_length, unsigned char *key_AES, unsigned char *key_HMAC, unsigned char *out_data, size_t *out_data_length){
