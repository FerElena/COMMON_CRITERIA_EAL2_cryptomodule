...
unsigned char FS_cipher_IV[AES_BLOCK_SIZE];
...
int API_FS_initiate_file_system(unsigned int mode , unsigned char *filesystem_route , size_t filesystem_route_length)
{
    ...
    if (mode == MODE_INIT) // we intialice the file for the first time, or we reset the current file_system
    {
        ...
        if (MetadataBlock.FS_data_descriptor == NULL){ // return error if cannot open file
    ...
    else if (mode == MODE_LOAD) // gets the metadata from the metadata file, into the MetadataBlock
    {
        ...
        if (auxf == NULL){ // return error if cannot open file
            pthread_mutex_unlock(&FS_mutex);
            return NO_FILESYSTEM_FILES;
        }
...
int API_FS_create_file_data(unsigned char *filename, size_t filename_length, unsigned char *data, size_t data_size, uint8_t isCSP)
{
    ...
    if(isCSP && MetadataBlock.cipher_mode == CIPHER_ON){
        AES_OFB_EncryptDecrypt(data,data_size,FS_cipher_key,AES_KEY_SIZE_256,FS_cipher_IV,data_buffer);
...
int API_FS_delete_file(unsigned char *filename,size_t filename_length)
{
    ...
    if(MetadataBlock.allocations[index].isCSP)
    {
        ...
        if(MetadataBlock.allocations[index].isCSP && MetadataBlock.cipher_mode == CIPHER_ON)
            AES_OFB_EncryptDecrypt(data_buffer, MetadataBlock.allocations[index].size,FS_cipher_key,AES_KEY_SIZE_256,FS_cipher_IV,data_buffer);
...
int API_FS_update_file_data(unsigned char *filename, size_t filename_length, unsigned char *data, size_t data_size) {
    ...
    if (data_size <= current_size) {  // if updated data fits in the current space of the file, we simply write the new data on the current offset
    ...
    else { // else, if we have to search for a new space in the filesystem (this fragmentates a lot the filesystem, this operation is not recomended)
        ...
        if (new_offset == MAX_SIZE_REACHED) { // if no more space avaiable in the file system for the updated
            pthread_mutex_unlock(&FS_mutex);
            return MAX_SIZE_REACHED;
        }
...
int find_space_for_data(size_t data_size,unsigned int exclude_index) { //auxiliar function for API_FS_update_file_data , it finds the best place for larger data

    int potential_start = 0;
    for (int i = 0; i < MetadataBlock.num_filenames; i++) {            //looks for potential start for new file from position 0
        if (i == exclude_index) 
            continue;
...
void API_FS_Close_filesystem(){  //easy functions to force close the file_system, ignoring other threads 
